        ...
        <div class="col-sm-12 grid-margin stretch-card">
            <div class="card">
                <div class="card-header">
                    <h5 class="mb-0">
                        <i class="mdi mdi-account-badge-horizontal"></i>&nbsp; Mes visites médicales
                ...
                <div class="card-body">
                    <h4 class="card-title">
                        ...
                        @if(Request::has(['date_debut','date_fin']))
                                <a class="float-sm-right text-primary" href="{{route('medvisites.index')}}"><i class="mdi mdi-filter-remove"></i>&nbsp; Réinitialiser</a>
                        @endif
                    </h4>
                            ...
                            @foreach($visites as $visite)
                                <tr>
                                    <td class="text-center">
                                        <a class="text-info" style="font-size: 1.6rem" href="#visite{{$visite->visitemed_id}}"  aria-controls="#visite{{$visite->visitemed_id}}" data-toggle="collapse" aria-expanded="false" ><i class="mdi mdi-chevron-down-circle"></i></a>
                                    </td>
                                    ...
                                    <td class="text-uppercase">{{$visite->medecin->etablissement}}</td>
                                    <td class="text-uppercase">{{$visite->medecin->potentiel}}</td>
                                    <td class="text-uppercase">{{$visite->medecin->ville->libelle}}</td>
                                    <td class="text-capitalize">{{$visite->etat}}</td>
                                    <td>
                                        <div class="dropdown">
                                            <button class="btn btn-sm btn-secondary dropdown-toggle" type="button" id="dropdown{{$visite->visitemed_id}}" data-toggle="dropdown" aria-haspopup="true" aria-expanded="false">Actions</button>
                                            <div class="dropdown-menu mt-0" aria-labelledby="dropdown{{$visite->visitemed_id}}">
                                                <a class="dropdown-item" href="{{route('medvisites.show',['medvisite' => $visite->visitemed_id])}}"><i class="mdi mdi-eye"></i>&nbsp;Voir</a>
                                                @if($visite->etat === "plan") <a class="dropdown-item" href="{{route('medvisites.edit',['medvisite' => $visite->visitemed_id])}}"><i class="mdi mdi-pencil"></i>&nbsp;Modifier</a> @endif
                                                @if($visite->etat === "plan") <a class="dropdown-item" data-toggle="modal" data-target="#deleteModel" data-whatever="{{$visite->visitemed_id}}" data-docteur="{{$visite->medecin->nom}} {{$visite->medecin->prenom}}" data-datev="{{Carbon\Carbon::createFromFormat('Y-m-d H:i:s',$visite->date_visite)->format('d/m/Y')}}"><i class="mdi mdi-trash-can"></i>&nbsp;Supprimer</a> @endif
                                            </div>
                                        </div>
                                    </td>
                                </tr>
                                <tr class="collapse table-info"  id="visite{{$visite->visitemed_id}}">
                                     <td colspan="4" >
                                               ...
                                               <ul class="mb-0">
                                                   @foreach($visite->products as $product)
                                                       <li class="pb-1">
                                                           ...
                                                           Feedback : <strong> {{$product->pivot->feedback->libelle}}  </strong> &nbsp;
                                                           Echantillon(s) donnés : <strong>{{$product->pivot->nbr_ech}}</strong>
                                                       </li>
                                                   @endforeach
                                                </ul>
                                           @else
                                                <span>il n'y a pas encore de produits présentés</span>
                                           @endif
    ...
    <div class="modal fade" id="filterModel" tabindex="-1" role="dialog" aria-labelledby="filterModel" aria-hidden="true">
        <div class="modal-dialog" role="document">
            <div class="modal-content">
                ...
                <div class="modal-body">
                    <form id="filter-form" action="{{route('medvisites.index')}}" method="GET">
...
<script>
    $(document).ready(function () {
        ...
        $('#deleteModel').on('show.bs.modal', function (event) {
            ...
            var docteur = button.data('docteur'); // Extract info from data-* attributes
            // If necessary, you could initiate an AJAX request here (and then do the updating in a callback).
            // Update the modal's content. We'll use jQuery here, but you could use a data binding library or other methods instead.
            var modal = $(this);
            modal.find('.modal-body>form').attr('action','/medvisites/'+v_id);
            modal.find('.modal-title>span').text(date_visite + " - Médecin: " + docteur);
